...
async function add(req: Request, res: Response) {
  ...
  try {
    ...
    if (!especialidad?.id) {
      return res
        .status(400)
        .json({ message: "Falta el ID de la especialidad" });
    }

    const especialidadObj = await em.findOneOrFail("Especialidad", {
      id: especialidad.id,
    });
...
async function remove(req: Request, res: Response) {
  try {
    ...
    // Solo contamos turnos PENDIENTES
    ...
    if (turnosPendientes > 0) {
      return res.status(409).json({
        message: `No se puede eliminar: tiene ${turnosPendientes} turno(s) pendiente(s).`,
      });
    }
...
async function update(req: Request, res: Response) {
  ...
  try {
    ...
    await em.flush();
    res.status(200).json({ message: "medico updated", data: medicoToUpdate });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
}
